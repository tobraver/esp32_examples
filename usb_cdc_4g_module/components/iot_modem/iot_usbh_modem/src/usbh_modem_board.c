...
typedef enum {
    ...
    STAGE_CHECK_ICCID = 4,  /* check integrated circuit card identifier */
    STAGE_CHECK_SIGNAL = 5, /* check net signal strength */
    STAGE_CHECK_REGIST = 6, /* check is net registered to operator */
...
static const _modem_stage_msg_t modem_stage_msg_table[] = {
    ...
    MODEM_STAGE_CODE2STR(STAGE_CHECK_ICCID),
                           ...
                           int32_t event_id, void *event_data)
{
    if (event_base == IP_EVENT) {
        ...
        if (event_id == IP_EVENT_PPP_GOT_IP) {
            ...
            memset(&s_modem_ip_info, 0, sizeof(s_modem_ip_info));
...
static bool _check_signal_quality()
{
    ...
    if (modem_board_get_signal_quality(&rssi, &ber) == ESP_OK) {
        if (rssi != 99 && rssi > 5) {
            ...
            s_modem_single.rssi = rssi;
            s_modem_single.ber = ber;
            return true;
        } else {
...
static bool _check_network_registration()
{
    ...
    if (modem_board_get_operator_state(operater_name, sizeof(operater_name)) == ESP_OK) {
        if (strlen(operater_name) > 0) {
            ...
            snprintf(s_modem_sim_name, sizeof(s_modem_sim_name), "%s", operater_name);
            return true;
        } else {
